...
def eular_to_transform(yaw_magnitude:float, pitch_magnitude:float, roll_magnitude:float, translation_magnitude:float) -> np.ndarray:
    euler = np.array([1., 1., 1.])*[yaw_magnitude, pitch_magnitude, roll_magnitude]* np.pi / 180.0
    rotation = Rotation.from_euler('zyx', euler).as_matrix()
    translation = np.random.uniform(-translation_magnitude, translation_magnitude, 3)
    transform = get_transform_from_rotation_translation(rotation, translation)
    return transform    
